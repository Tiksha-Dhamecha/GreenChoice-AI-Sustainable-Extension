(function () {
  ...
  /* -----------------------------------------
     ORDER HISTORY DETECTION (Simple Amazon Demo)
  ------------------------------------------*/

  function scanAmazonOrders() {
    // Only run on Amazon Order History pages
    const isAmazon = location.hostname.includes('amazon');
    const isOrdersPage = (location.href.includes('order-history') || location.href.includes('your-orders') || location.href.includes('orders'));

    if (!isAmazon || !isOrdersPage) return;

    console.log("[GreenChoice] Scanning Amazon orders page...");

    // Ensure User ID
    chrome.storage.local.get("greenchoice_user_id", (items) => {
      let userId = items.greenchoice_user_id;
      if (!userId) {
        userId = "user_" + Math.random().toString(36).substr(2, 9);
        chrome.storage.local.set({ greenchoice_user_id: userId });
      }

      // Select all potential order cards
      // .a-box-group is the standard container for an order in 'Your Orders'
      const orderCards = document.querySelectorAll('.js-order-card, .yohtml5-order-card, .a-box-group');

      console.log(`[GreenChoice] Found ${orderCards.length} potential order cards.`);

      orderCards.forEach((card, index) => {
        const text = (card.innerText || "").toLowerCase();

        // Skip cards that are just headers or filters
        if (!text.includes("order") && !text.includes("placed")) {
          // console.log(`[GreenChoice] Card ${index} skipped: no 'order/placed' text`);
          return;
        }

        let status = "placed";

        // Debug: Log the text for the first few cards to understand why they are 'placed'
        if (index < 3) {
          console.log(`[GreenChoice] Card ${index} text snippet:`, text.substring(0, 150).replace(/\n/g, " "));
        }

        // Broader status detection
        // "Return window closed" or "Return eligible" implies the item was delivered.
        if (
          text.includes("delivered") ||
          text.includes("handed to resident") ||
          text.includes("left near door") ||
          text.includes("was dropped off") ||
          text.includes("return window closed") ||
          text.includes("return eligible") ||
          text.includes("window closed on")
        ) {
          status = "delivered";
        }
        else if (text.includes("return complete") || text.includes("refunded") || text.includes("refund issued")) status = "returned";
        else if (text.includes("cancelled")) status = "cancelled";
        else if (text.includes("shipped") || text.includes("arriving") || text.includes("out for delivery")) status = "shipped";

        // Extract Order ID
        let orderId = null;
        const oidEl = card.querySelector('.yohtml5-order-id span') || card.querySelector('bdi') || card.querySelector('.a-color-secondary.value');
        if (oidEl) orderId = oidEl.innerText.trim();

        if (!orderId) {
          // "Order # 123-..."
          const m = text.match(/order\s*#\s*([0-9-]{15,}|[0-9-]{17,})/i);
          if (m) orderId = m[1];
          else {
            // Try just finding the pattern 123-1234567-1234567
            const m2 = card.innerText.match(/\b\d{3}-\d{7}-\d{7}\b/);
            if (m2) orderId = m2[0];
          }
        }

        // Extract Title
        const titleLink = card.querySelector('a[href*="/dp/"], a[href*="/product/"], .yohtml5-product-title, .a-link-normal[href*="product"]');
        let title = titleLink ? titleLink.innerText.trim() : null;

        // Fallback for title: look for text that isn't a button or label
        if (!title) {
          // This is risky but helps if DOM is obscure. 
          // Try the first link that has substantial text and isn't "View order details"
          const links = card.querySelectorAll('a');
          for (const l of links) {
            const t = l.innerText.trim();
            if (t.length > 10 && !t.includes("order") && !t.includes("track") && !t.includes("invoice") && !t.includes("buy it again")) {
              title = t;
              break;
            }
          }
        ...
        if (!orderId || !title) {
          console.log(`[GreenChoice] Card ${index} skipped: Missing ID (${!!orderId}) or Title (${!!title}). ID: ${orderId}`);
          return;
        }

        // Determine effective sustainability logic on backend, but we need to send the data first.
        // We send "placed" orders too? No, requirement says "Streak update only after delivery confirmation".
        // But if we want to debug, we should log "Found valid card ... but status is placed"

        if (status === "delivered" || status === "returned" || status === "cancelled") {
          console.log(`[GreenChoice] Syncing verified order: ${orderId} (${status}) - ${title}`);
          ...
          chrome.runtime.sendMessage({
            ...
            if (chrome.runtime.lastError) return; // ignore common messaging errors
            if (response && response.success && response.data) {
              const d = response.data;
              if (d.streak_awarded) {
                console.log(`[GreenChoice] STREAK INCREASED! New total: ${d.current_streak}`);
                // Visual flair on the card? 
                card.style.border = "3px solid #22c55e"; // Green border for streak items
              } else if (d.order_status === "delivered") {
                // Delivered but not sustainable enough?
                console.log(`[GreenChoice] Order synced. Streak not increased (Score < 5?). Streak: ${d.current_streak}`);
              }
            }
          });
        } else {
          console.log(`[GreenChoice] Card ${index} valid but status '${status}' (not Delivered/Returned) - Skipping sync.`);
        }
      });
    });
  }
  const scanDebounced = debounce(scanAmazonOrders, 2000);

  // Initial scan
  setTimeout(scanAmazonOrders, 3000);

  // Watch for dynamic content changes (pagination, filters)
  ...
  const container = document.getElementById('yourOrdersContent') || document.body;
  if (container) {
