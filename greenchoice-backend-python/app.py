...
# def alternatives():
    ...
    Expects JSON { "products": [string or {title, description, url, ...}] }
    Uses a SINGLE Groq call for all products for speed.
    """
    payload = request.get_json(silent=True) or {}
    ...
    # 1) Normalize to a clean list of names (strings)
    names = []
    for p in products[:8]:   # limit to top 8 for speed
    ...
    # 2) Try fast AI bulk scoring
    try:
        ai_list = ai_score_alternatives(names)  # ONE Groq call
        # Build a map from name -> ai result
        ai_map = {}
        for item in ai_list:
            n = item.get("name")
            if not n:
        ...
        # 3) For each original name, assemble final result (with heuristic fallback for any missing)
        for name in names:
            ai_item = ai_map.get(name)
            if ai_item:
                results.append({
                    "name": name,
                    "numericScore": ai_item.get("numericScore"),
                    "grade": ai_item.get("grade"),
                })
            else:
                # fallback for that one item
        ...
        # 4) If bulk AI fails entirely, fall back to heuristic for all
        print("AI bulk failed for alternatives, using fallback:", e, flush=True)
        for name in names:
            numeric_score = compute_heuristic_score(name)
            results.append({
                "name": name,
